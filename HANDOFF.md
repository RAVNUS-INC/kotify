...
- 남은 사항: SMS 대체 발송을 기다리는 동안 SQLite 쓰기 트랜잭션을 유지하는 기존 구조 문제와 최초 발송 `ComposeForm`의 UTF-8 예상 길이/서버 EUC-KR 불일치는 이번 범위 밖으로 남긴다. 로컬 수정·검증은 완료했으며 게시·운영 적용은 별도 작업이다. 배포 시 아래 운영 절차와 `deploy/README.md`의 `0019` 호환 안내를 따른다.
