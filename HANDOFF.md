...
- 배포 상태: 아직 통합 브랜치이며 `main` 푸시·운영 배포 전. 최종 검사 후 기존 업데이트 스크립트로 배포하고 두 서비스 및 버전 헬스체크를 확인한다.
- 남은 단계: 배포 후 대상 사용자의 로그아웃·재로그인에서 진단 필드와 최종 역할을 대조한다. 원인이 확인되면 앱 파서·Keycloak 매퍼·기존 세션 중 해당 지점을 수정한다. 상세 조회 및 원인별 조치는 `deploy/README.md`를 따른다.
