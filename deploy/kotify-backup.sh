#!/usr/bin/env bash
# deploy/kotify-backup.sh
# SQLite DB 일일 백업 스크립트
# 실행 계정: kotify (cron에서 자동 실행)
# 실행 시각: 매일 00:00 (kotify-backup.cron 참고)
#
...
# ── 설정 ─────────────────────────────────────────────────────────────────────
DB_SOURCE="/var/lib/kotify/sms.db"
BACKUP_DIR="/var/backups/kotify"
RETENTION_DAYS=30
TIMESTAMP=$(date +%Y%m%d)
BACKUP_FILE="${BACKUP_DIR}/kotify-${TIMESTAMP}.db"
LOG_TAG="kotify-backup"
...
done < <(find "${BACKUP_DIR}" -name "kotify-*.db" -mtime "+${RETENTION_DAYS}" -type f)

if [[ ${DELETED_COUNT} -gt 0 ]]; then
    log_info "${DELETED_COUNT}개의 오래된 백업 파일 삭제 완료"
fi

log_info "백업 완료. 보관 중인 백업 수: $(find "${BACKUP_DIR}" -name "kotify-*.db" -type f | wc -l)"
