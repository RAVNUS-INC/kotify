...
변경 계약과 `0019` 호환·배포 절차는 `claudedocs/SPEC.md` §4.3·§8.4와 `deploy/README.md`에 반영했다. 구 프런트엔드의 본문 없는 읽음 요청은 422가 되므로 백엔드·프런트엔드를 함께 배포하고 열린 탭을 새로고침해야 한다. SQLite 대체 발송 쓰기 잠금 구조 개선은 별도 미해결 사항으로 남긴다.
