...
> **전체 결과 코드 테이블(공통/인증/API/RCS/KKO/RBC 포함 200+개)은 별도 레퍼런스 참조:**
> [`msghub-error-codes.md`](./msghub-error-codes.md)
>
> 이 섹션은 자주 보는 요약만 유지한다.
...
|------|------|
| `10000` | 성공 |
| `29003` | 기타 오류 — 실 원인이 error 필드에 동봉 (예: `404 NOT_FOUND` = 챗봇/메시지베이스 미등록) |
| `29011` / `29025` | 인증/IP 권한 없음 → 토큰 재발급 필요 |
| `29021` | RCS 미지원 단말 → fallback 대상 |

### 인증 에러 (HTTP 401)

`20000` ~ `20004`, `29011`, `29025`

### 재시도 조건 (공식 정의)

- **CPS 초과**: `29002`
- **내부 오류**: `49xxx` (전체), `21400`, `22004`, `23004`, `23005`, `29017`, `29019`, `29032`, `31112`, `31118`, `65999`
- **RCS 발송 실패이나 단말 전달 가능성**: `41007`, `54004`, `55806`, `55820`
- **HTTP 5xx 전체**

앱 내 구현: `app/msghub/codes.py` 의 `RETRYABLE_CODES` + `_RETRY_PREFIXES=("49",)` +
`client.py` 의 HTTP 5xx 분기.
