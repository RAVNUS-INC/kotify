...
- publish_throttled: 새로고침 1회가 목록(list_threads)·상세 API 와 하이웍스 외부 MySQL
  조회를 다시 부르고 열린 탭 수만큼 곱해진다. 대량 발송 리포트와 회신을 부르는 캠페인의
  MO 는 몰려오므로 이벤트 이름마다 창(window) 안에서 한 번만 보낸다 — 조용하던 뒤 첫
  변경은 즉시(leading), 창 안의 나머지는 창 끝에 한 번(trailing). 끝에 한 번 더 보내므로
  몰려온 변경의 마지막 것도 창 길이 안에 반영된다.
"""
